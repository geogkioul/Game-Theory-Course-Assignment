% Example 17 - N = 5, ALLD-TFT-PROBER
...
POP0 = [0,2,1,0,0,0,0,0,2,0]; % Initial population matrix
...
% % Make a legend box to translate state numbers (graph nodes) to state
% % vectors (populations of active strategies).
% % Join the active strategy names into a single string
% active_strats_str = strjoin(active_strategies, ', ');
% 
% % Make a bold title for the annotation box
% legend_title = sprintf('\\bf Active strategies: %s\\rm', active_strats_str);
% % Build the state number → state vector lines
% legend_lines = strings(S, 1);
% for i = 1:S
%     vec_str = sprintf('%d ', states(i, :));
%     legend_lines(i) = sprintf('State %2d → [%s]', i, strtrim(vec_str));
% end
% 
% % Combine title + legend lines
% legend_text = strjoin([legend_title; legend_lines], newline);
% 
% % Plot the annotation (legend box)
% annotation('textbox', [0.72 0.1 0.25 0.8], ...
%            'String', legend_text, ...
%            'FontName', 'Courier', ...
%            'FontSize', 9, ...
%            'Interpreter', 'tex', ...

...
function [POP, BST, FIT] = TourSimFit(B, Strategies, POP0, T, J)
    ...
    % We will perform a PARTIAL simulation. That is, we won't let the
    % players play each match, but since the strategies are deterministic
    % we will compute the payoffs of their matches beforehand.
    % That way we will save much computation time
    
    % Simulate the tournament for each generation
    for gen = 1:J
        scores = axel_partial(B, Strategies, POP(:, gen), T); % Scores after simul
